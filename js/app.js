...
window.addEventListener(
  ...
  () => {
    ...
    if (name.length == 0) {
      ...
      div.innerHTML = `
      <p class="mt-0 mb-1 mx-0 p-0 text-white">Kepada Yth Bapak/Ibu/Saudara/i</p>
...
function tampilkanAtauTutupOverflowXHidden() {
  ...
  if (overflowXHiddenDiv) {
    if (isOverflowXHiddenVisible) {
      // Jika elemen sudah ditampilkan, sembunyikan kembali dengan menambahkan kelas "hidden-before-click"
      overflowXHiddenDiv.classList.add("hidden-before-click");
      isOverflowXHiddenVisible = false;
    } else {
      // Jika elemen belum ditampilkan, tampilkan dengan menghapus kelas "hidden-before-click"
